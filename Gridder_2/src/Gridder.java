...
public class Gridder<grid> extends javax.swing.JFrame
                                        implements MouseListener, MouseMotionListener,
                                                            ActionListener {
    Random rand = new Random();
    protected final int gridCount=100;  //how many squares wide our drawing area is
    private int[][] grid = new int[gridCount][gridCount]; //create the matrix
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        // create col of white squares on click

        for(int x = 0; x<=100; x++)
        {
            int col = rand.nextInt(gridCount);
            int row = rand.nextInt(gridCount);
            //clickGrid(col,row);
    ...
    }//GEN-LAST:event_jB/utton1ActionPerformed
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        System.out.println("2");
        int newCol = rand.nextInt(gridCount);

        //create row of white squares on click
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        ...
        for(int newRow =gridCount/2; newRow<gridCount; newRow++)
        {
            for(int newCol = 0; newCol<gridCount; newCol++)
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        int count = 0;
//method "print # of white squares to gridder gui"
        for(int row = 0; row<gridCount; row++)
            for(int col = 0; col < gridCount; col++)
            {
                if(grid[col][row] == 1)
    ...
    private void jButton5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        for (int newRow = 0; newRow < gridCount; newRow++)
            for (int newCol = 0; newCol < gridCount; newCol++)
                if(newCol==newRow || newCol==gridCount-newRow-1)
...
//and finally, make grid equal temp and draw
    ...
    private void jButton7ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        //declare temp array, copy array "grid" into temp
        int[][] temp = new int[100][100];
        for (int row=0; row<100; row++)
        ...
        for(int row = 1; row<99; row++)
            for(int col = 1; col<99; col++) {
                //cases for "if black square touches 2 white squares make black square white
                if(grid[col][row] == 1)
                     count++;

                if(grid[col-1][row-1] == 1)
                    count++;
                if(grid[col+1][row+1] == 1 && row<99 && col<99)
                    count++;
                if(grid[col+1][row] == 1 && col<99)
                    count++;
                if(grid[col-1][row] == 1)
                    count++;
                if(grid[col][row+1] == 1 && row<99 )
                    count++;
                if(grid[col][row-1] == 1)
                    count++;
...
//every time we get a white one reduce col by 1
        //ie with col 1 move to col 99 turn 99 to white 1 to black
        for (int row=1; row<gridCount; row++)
            for (int col=1; col<gridCount; col++) {
                //int newCol = gridCount-col;
                if (grid[col][row]==1) //check to the left
                    temp[col-1][row] = 1;
                    temp[col][row] = 0;
                //if(grid[col][row] == 1)
                  //  temp[col][row] = 0;
...
//end edge!
    ...
    private void jButton11ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton11ActionPerformed
        ...
        int count = 0;
        for(int row = 1; row<gridCount; row++)
            for(int col = 1; row<gridCount; col++) {

                if(grid[col][row] == 1 && grid[col-1][row-1] == 1)
                    count++;
                if(grid[col][row] == 1 && grid[col+1][row+1] == 1)
                    count++;
                if(grid[col][row] == 1 && grid[col+1][row] == 1)
                    count++;
                if(grid[col][row] == 1 && grid[col-1][row] == 1)
                    count++;
                if(grid[col][row] == 1 && grid[col][row+1] == 1)
                    count++;
                if(grid[col][row] == 1 && grid[col][row-1] == 1)
                    count++;
                if(count>=2)
